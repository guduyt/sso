...
public class VerifyCodeGenerator {
    ...
    public VerifyCodeGenerator(){
        text=generatorText(4);
        bufferedImage=getGeneratorImage(text);
    }
    ...
    private String generatorText(int number){
        StringBuilder sb=new StringBuilder();//用来装载生成的验证码文本
//向图中画4个字符
        for(int i=0;i<number;i++){//循环4次
    ...
    //外界调用这个方法得到验证码
    private BufferedImage getGeneratorImage(String text){
        BufferedImage image=createImage();//创建图片缓冲区
        Graphics2D g2=(Graphics2D)image.getGraphics();//得到绘制环境
        for(int i=0;i<text.length();i++){//循环4次
            String s=text.substring(i,i+1);//随机生成一个字符
